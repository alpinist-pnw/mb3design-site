...
**Work order:**
1. **C2M canonicalization** — **GATE PASSED 2026-08-13 eve; Phase 2 wave 1 EXECUTING.** Gate outcomes in `.c2m-triage/PHASE1_REPORT.md` § GATE OUTCOMES: MB3 job numbers minted (M100 Sails / M101 DOTS / M000 standards — 23111-* ruled date codes, not job numbers); winners-only dedup; SHARED → both projects; month subfolders for unnamed media; 1871 bid → new `05-RFP Vault/`; all 2022 videos ruled personal. Wave 1 = 435 hash-verified copies (20.6 GB) into `01-Projects Vault/C2M/`, log `copy_log.csv`. **Wave 2 pending Mark's checkbox pass** on `triage.html` (1,272 unnamed photos/videos: personal checkbox, SAILS/DOTS selector date-prefilled, type tags) served at `127.0.0.1:8643` — session-bound server, restart: `python3 .c2m-triage/server.py &`. Then Phase 3 verify + provenance README.
